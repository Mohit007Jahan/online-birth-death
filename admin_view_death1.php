...
try {
    ...
    $stmt = $conn->query("SHOW TABLES LIKE 'tbldeathapplications'");
    if ($stmt->rowCount() > 0) {
        echo "<!-- Table 'tbldeathapplications' exists -->\n";
        
        // Get table structure
        $stmt = $conn->query("DESCRIBE tbldeathapplications");
        $columns = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo "<!-- Table structure:\n" . print_r($columns, true) . "\n-->\n";
        
        // Get record count
        $stmt = $conn->query("SELECT COUNT(*) FROM tbldeathapplications");
        $count = $stmt->fetchColumn();
        echo "<!-- Total records in table: " . $count . " -->\n";
    } else {
        echo "<!-- Table 'tbldeathapplications' does not exist. Available tables are listed above. -->\n";
    }
...
// Get total count for pagination
    $count_query = "SELECT COUNT(*) FROM tbldeathapplications a LEFT JOIN tbluser u ON a.user_id = u.ID";
...
<head>
    ...
    <title>View Death Applications</title>
    ...
    <style>
        ...
        * {
        ...
        /* Keep all existing death certificate view styles */
        .main-content {
            flex-grow: 1;
            padding: 20px;
        }
...
<body>
                    ...
                    <tbody>
                        ...
                        <?php foreach ($applications as $app): ?>
                            <tr>
                                    ...
                                    <td><?php echo isset($app['date_of_death']) ? date('M d, Y', strtotime($app['date_of_death'])) : 
                                               (isset($app['DATE_OF_DEATH']) ? date('M d, Y', strtotime($app['DATE_OF_DEATH'])) : 
                                               (isset($app['DateOfDeath']) ? date('M d, Y', strtotime($app['DateOfDeath'])) : 'N/A')); ?></td>
                                ...
                                <td>
                                        <a href="admin_view_death_application.php?id=<?php echo $app['id'] ?? $app['ID'] ?? $app['Id']; ?>" 
    ...
    <script>
        ...
        function applyFilters() {
            ...
            window.location.href = `admin_view_death1.php?status=${status}&search=${search}`;
        }
